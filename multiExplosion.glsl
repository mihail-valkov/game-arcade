...
const float particles = 50.0; // change particle count
...
uniform vec2 explosionSizes[MAX_EXPLOSIONS]; // array of explosion positions
...
float explosion(vec2 uv, float time) {
    ...
    for (float i = 0.0; i < particles; i++) {
        ...
        float d = length(uv - dir * time *.1);
        float brightness = mix(0.0005, 0.002, smoothstep(0.05, 0.0, time));
        brightness *= sin(time * 20.0 + i) *.5 + .5;
        brightness *= smoothstep(1., .5, time);
        sparks += brightness / d;
    }
    return sparks;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 explodePosition = explodePositions[0];
    float explodeTime = explodeTimes[0];
    ...
    vec3 col = vec3(0.0);
    col += explosion(explosionCenter, explodeTime) * //vec3(1., 0.647, 0.3);
      vec3(1., smoothstep(0.647, 0.0, explodeTime), smoothstep(0.3, 0.0, explodeTime)); // organge to red
